...
app.post('/upload', uploadFile.single('upload'), (reqL, resL) => {
    ...
    .then(res => {
        if (res.data.result) {
            ...
            const html_str = 
            head()+`<body>`+body_header()+
            `<main class="main-container">`+query_container(query_path, query_name)+ make_cctv_container(gallery_path, gallery_path_, h_id)+`</main>`
            +`</body>`+`</html>`
...
async function returnPromiseCCTV(cctv_list, cctv_containers_list) {
    return new Promise((resolve, reject) => {
        for (let i = 0; i < cctv_list.length; i++) {
            let cctv_container = `<article class="cctv-container">`+cctv_container_header(cctv_list[i])+`</article>`
            // cctv_containers_list.push(cctv_container)
            resolve(cctv_container)
        }
    })
}
...
const make_cctv_container = (gallery_path, gallery_path_, h_id)=> {
    let cctv_containers_list = new Array()
    fs.readdir(`${gallery_path_}/${h_id}`, (err, cctv_list) => {
        // for (let i=0; i<cctv_list.length; i++) {
            returnPromiseCCTV(cctv_list, cctv_containers_list)
            .then((result) => {
                console.log(result);
                return result
              })
...
const gallery_container = async (gallery_path, gallery_path_, h_id) => {
    ...
    // let result = ''
    // await Promise.all(
    //     returnPromise(container_string,gallery_path, gallery_path_, h_id))
